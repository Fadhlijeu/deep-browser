...
app.setAboutPanelOptions({
  applicationName: 'Deep-Browser',
  applicationVersion: app.getVersion(),
  copyright: '© 2026 Deep-Browser Contributors',
  website: 'https://github.com/Fadhlijeu/deep-browser',
});
...
app.whenReady().then(async () => {
  ...
  // ---------------------------------------------------------------------------
  // Onboarding gate — Deep-Browser bypasses vendor setup and opens Hub directly
  // ---------------------------------------------------------------------------
  const forceOnboarding = process.env.AGB_FORCE_ONBOARDING === '1';
  const onboardingComplete = !forceOnboarding; // Deep-Browser default: open Hub directly
  if (!accountStore.isOnboardingComplete()) {
    accountStore.save({
  ...
  // Register onboarding + chrome-import IPC once at app boot.
  ...
  if (!onboardingComplete) {
    ...
    mainLogger.info('main.onboardingGate.returning', { msg: 'Launching Deep-Browser Hub shell' });
    openShellAndWire();
  }
